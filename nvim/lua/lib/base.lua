...
local function get_merge_base(other_branch)
    return utils.system({ 'git', 'merge-base', 'HEAD', other_branch })
end

function M.get_base_merge_base()
    local base_branch = utils.system({ 'git', 'get-base' })
    if base_branch == '' then
        base_branch = utils.system({ 'gh', 'pr', 'view', '--json', 'baseRefName', '-q', '.baseRefName' })
    end
    if base_branch == '' then
